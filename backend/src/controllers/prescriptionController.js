...
const { sendFollowUpEmail, sendSMS } = require("../utils/notificationService");
...
exports.createPrescription = async (req, res) => {
  try {
    ...
    // Create follow-up document if followUpDate is provided
    if (followUpDate) {
      ...
      // Send follow-up notification email and SMS
      const dashboardLink = process.env.PATIENT_DASHBOARD_URL || "http://localhost:3000/patient/dashboard";
      const confirmLink = `${dashboardLink}/followups/${followUp._id}/confirm`;
      const receptionistNumber = process.env.RECEPTIONIST_PHONE || "+1-800-NIRAM-AI";
      ...
      await sendSMS(
        ...
        `Dear ${appointment.patientId.name}, Your doctor has recommended a follow-up appointment on ${new Date(followUpDate).toLocaleDateString()}. Please confirm at ${dashboardLink} or call ${receptionistNumber}`
      );
    }
