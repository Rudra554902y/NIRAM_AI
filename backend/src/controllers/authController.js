...
exports.login = async (req, res) => {
  try {
    ...
    // If user is a doctor, check if they're a super doctor
    if (user.role === "DOCTOR") {
      const Doctor = require("../models/Doctor");
      const doctorProfile = await Doctor.findOne({ userId: user._id });
      if (doctorProfile) {
        userData.isSuperDoctor = doctorProfile.isSuperDoctor || false;
      }
    }
...
exports.googleCallback = async (req, res) => {
  try {
    ...
    // Check if user is a doctor and get super doctor status
    let isSuperDoctor = false;
    if (user.role === "DOCTOR") {
      const Doctor = require("../models/Doctor");
      const doctorProfile = await Doctor.findOne({ userId: user._id });
      if (doctorProfile) {
        isSuperDoctor = doctorProfile.isSuperDoctor || false;
      }
    }
    ...
    // Redirect to frontend with tokens in URL (will be handled by frontend and stored securely)
    const redirectUrl = process.env.FRONTEND_URL || "http://localhost:3000";
    res.redirect(`${redirectUrl}/auth/google/callback?token=${encodedToken}&user=${encodedUserData}`);
  } catch (error) {
